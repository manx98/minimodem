...
#if WIN32
void tx_stop_transmit_sighandler()
#else
...
)
{
...
#if WIN32
	long int itv = 1000000 / (float)(data_rate + data_rate * 0.03f);
	init_win_setitimer();
#else
	// one-shot
	...
	while (!end_of_file)
	{
		...
		{
#if WIN32
			win_setitimer(0, &tx_stop_transmit_sighandler);
#else
		...
		{
#if WIN32
			win_setitimer(itv, &tx_stop_transmit_sighandler);
#else
...
#endif
	if ((unsigned long long)(nbits_decoded * sample_rate + 0.5f) == (unsigned long long)(bfsk_data_rate *
		carrier_nsamples))
	{
...
benchmarks()
{
	...
	if (ret); // don't care, hush compiler.
...
#if !(WIN32 || USE_SNDIO || USE_PULSEAUDIO || USE_ALSA)
...
# endif
	...
	if (filename)
	{
		...
		fprintf(
			stderr,
			"E: This build of minimodem was configured without sndfile,\nE:   so the --file flag is not supported.\n");
		exit(1);
#endif
	}
	else
	{
		...
		fprintf(
			stderr,
			"E: this build of minimodem was configured without system audio support,\nE:   so only the --file mode is supported.\n");
		exit(1);
#endif
	}
...
#define SAMPLE_BUF_DIVISOR 12
	...
	if (expect_data_string == NULL)
	{
		...
		expect_n_bits = build_expect_bits_string(expect_data_string, bfsk_nstartbits, bfsk_n_data_bits, bfsk_nstopbits,
	...
	if (expect_sync_string == NULL && bfsk_do_rx_sync && (long long)bfsk_sync_byte >= 0)
	{
		...
		build_expect_bits_string(expect_sync_string, bfsk_nstartbits, bfsk_n_data_bits, bfsk_nstopbits,
		                         invert_start_stop, 1, bfsk_sync_byte);
	}
	else
	{
...
#define FSK_ANALYZE_NSTEPS_FINE		8
		...
		debug_log("Input: %08x%08x - Databits: %u - Shift: %i\n", (unsigned int)(bits >> 32), (unsigned int)bits,
		          bfsk_n_data_bits, bfsk_nstartbits);
